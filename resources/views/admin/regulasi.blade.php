...
<style>
    ...
    .header-section h1 { color: #001e74; font-size: 32px; font-weight: 700; position: relative; padding-bottom: 12px; margin: 0; letter-spacing: 0.5px; flex-shrink: 0; } 
    .header-section h1::after {
        content: ''; position: absolute; left: 0; bottom: 0; width: 60px; height: 5px;
    ...
    .header-section h1:hover::after { width: 100%; box-shadow: 0 6px 20px rgba(250, 204, 21, 0.7); }

    .add-button-group { display: flex; align-items: center; gap: 10px; flex-shrink: 0; } 
    .btn-add-file {
        background-color: #28a745; color: white; padding: 10px 18px; border-radius: 8px; text-decoration: none;
    ...
    .filter-section {
        background-color: #ffffff; padding: 25px 30px; border-radius: 12px;
    ...
    .filter-section .form-control {
        ...
        background-color: #f9fafb; font-size: 15px; color: #333; appearance: none;
    ...
    .filter-section input.form-control {
        background-color: #f9fafb; border: 1px solid #d1d5db; border-radius: 8px;
        padding: 10px 15px; font-size: 15px; color: #333; transition: border-color 0.3s ease, box-shadow 0.3s ease; width: 100%;
    }
    .filter-section input.form-control:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.25); outline: none; }
    .btn-filter-submit {
        background-color: #001e74; color: white; padding: 10px 25px; border-radius: 8px;
    ...
    .btn-view-doc {
        background-color: #3b82f6; display: inline-flex; align-items: center; gap: 8px;
        color: white; padding: 8px 12px; border-radius: 8px; text-decoration: none;
        font-weight: 600; font-size: 12px; border: none; outline: none;
    ...
    .btn-action-icon {
        ...
        border-radius: 8px; border: 1px solid; cursor: pointer; font-size: 15px;
        transition: background-color 0.3s ease, transform 0.2s ease, border-color 0.3s ease; box-sizing: border-box;
    }
    .btn-action-icon.edit { background-color: #facc15; color: #333; border-color: #facc15; }
    .btn-action-icon.edit:hover { background-color: #eab308; border-color: #eab308; transform: scale(1.05); }
    .btn-action-icon.delete { background-color: #ef4444; color: #fff; border-color: #ef4444; }
    .btn-action-icon.delete:hover { background-color: #dc2626; border-color: #dc2626; transform: scale(1.05); }
    ...
    .modal-content {
        background-color: #fff; margin: auto; padding: 30px; border-radius: 12px;
    ...
    @media (max-width: 768px) {
        ...
        .header-section { flex-direction: row; align-items: center; justify-content: space-between; gap: 10px; } /* Changed to row */
        .header-section h1 { font-size: 26px; text-align: left; width: auto; flex-grow: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } /* Adjustments for better horizontal flow */
        .header-section h1::after { left: 0; transform: translateX(0); width: 60px; } /* Keep line under h1 fixed */
        .add-button-group { width: auto; justify-content: flex-end; margin-top: 0; flex-shrink: 0; } /* Adjusted for horizontal alignment */
        .btn-add-file { width: auto; padding: 8px 15px; font-size: 14px; white-space: nowrap; } /* Allow button to shrink */
        ...
        .data-table tbody tr {
            margin-bottom: 15px; border: 1px solid #e0e0e0; display: flex; flex-wrap: wrap;
            padding: 15px; align-items: center;
        }
        .data-table td {
            border-bottom: 1px solid #eee; display: block; text-align: right;
            font-size: 14px; padding-left: 50%; position: relative; box-sizing: border-box;
            width: 100%;
        }
        ...
        .data-table .action-cell { justify-content: flex-start; width: 100%; margin-top: 10px; }
        .btn-view-doc { width: auto; flex-shrink: 0; }

        /* Responsive Filter Section */
        .filter-section { flex-direction: column; align-items: stretch; gap: 15px; padding: 20px; }
    ...
    @media (max-width: 480px) {
        ...
        .header-section h1 { font-size: 20px; } /* Further reduce font size for very small screens */
        .btn-add-file { font-size: 13px; padding: 6px 12px; }

        .filter-section { gap: 10px; padding: 15px; }
        .filter-group label { font-size: 13px; }
        .filter-section .form-control, .filter-section input.form-control { padding: 8px 12px; font-size: 14px; }
        .btn-filter-submit { font-size: 14px; padding: 8px 20px; }

        .data-table td { font-size: 13px; padding-left: 45%; }
        .data-table td::before { width: 40%; font-size: 13px; }
        .action-cell { gap: 5px; }
        .btn-view-doc { padding: 6px 10px; font-size: 11px; gap: 5px; }
        .btn-action-icon { width: 30px; height: 30px; font-size: 13px; }
    }
</style>
@endsection
