...
@section('content')
    ...
    <div class="berita-container">
        <h1 class="berita-title">{{ $evaluation->title }}</h1>

        <div class="berita-img-container">
            <div class="berita-actions">
                <a href="{{ route('admin.evaluasi.edit', $evaluation->id) }}" class="btn-edit"><i class="fas fa-pen"></i></a>
                <form action="{{ route('admin.evaluasi.destroy', $evaluation->id) }}" method="POST" onsubmit="return confirm('Yakin ingin hapus evaluasi ini?')">
                    @csrf
                    @method('DELETE')
                    <button class="btn-delete"><i class="fas fa-trash"></i></button>
                </form>
            </div>
            <img src="{{ asset('uploads/evaluasi/' . $evaluation->image) }}" alt="Gambar Evaluasi" class="berita-img">
        </div>
